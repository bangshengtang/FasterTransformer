...
int main(int argc, char* argv[])
{
  ...
  for (int device_id = 0; device_id < 2; device_id++) {
    threads.push_back(
      std::make_unique<std::thread>([device_id, argv, batch_size, num_layers, seq_len, head_num, size_per_head, is_remove_padding, int8_mode, allow_gemm_test]
        {
                    ...
                    bool allow_gemm_test)
{
  ...
  fastertransformer::Allocator<AllocatorType::CUDA> allocator(device_id);
  ...
  printf("[INFO] device_id %d batch_size %d seq_len %d layer %d FT-CPP-time %.2f ms ( %d iterations) \n", device_id, batch_size, seq_len, num_layers,
